    ...
    <Fade triggerOnce={true} style={{ margin: "20px auto" }}>
      <Card>
        ...
        {order.status === "pending" && (
          <OrderItemButtonContainer>
            <Button
              $buttonstyle={`${ORDERSTATUS.IN_PROCESS}`}
              onClick={() => dispatch(addStatusToProcess(order.id))}
            >
              En proceso
            </Button>
            <Button
              $buttonstyle={`${ORDERSTATUS.CANCELLED}`}
              onClick={() => dispatch(addStatusToCancelled(order.id))}
            >
              Cancelar
            </Button>
          </OrderItemButtonContainer>
        ...
        {order.status === "in process" && (
          <OrderItemButtonContainer>
            <Button
              $buttonstyle={`${ORDERSTATUS.COMPLETED}`}
              onClick={() => dispatch(addStatusToCompleted(order.id))}
            >
              Completar
            </Button>
            <Button
